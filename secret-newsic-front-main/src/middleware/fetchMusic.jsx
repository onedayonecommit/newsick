...
export const fetchGenreList = createAsyncThunk("music/fetchGenreList", async (genre) => {
  try {
    const genreList = await axios.get(`http://localhost:8080/music-genre-list/?genre=${genre}`);
...
//   try {
//     const likeMusicList = await axios.get("http://localhost:8080/playlist/", user_wallet_address);
...
export const fetchSongDetailInfo = createAsyncThunk("music/fetchSongDetailInfo", async (songId) => {
  try {
    const SongDetailInfo = await axios.get(`http://localhost:8080/music-detail-info/${songId}`);
...
export const fetchPlayList = createAsyncThunk("music/fetchPlayList", async (user_wallet_address) => {
  try {
    const playList = await axios.post("http://localhost:8080/playlist", user_wallet_address);
...
export const fetchAddSong = createAsyncThunk("music/fetchAddSong", async (addPlayList) => {
  try {
    ...
    const addSong = await axios.post("http://localhost:8080/playlist/add", addPlayList);
...
//   try {
//     const deleteSong = await axios.get(`http://localhost:8080/music-detail-info/?songNum=${songId}`);
