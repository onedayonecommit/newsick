  ...
  async (mypageData) => {
    try {
      const result = await axios({
        method: "post",
        url: "http://127.0.0.1:8080/mypage/second",
        data: mypageData,
      });
      return result.data;
    } catch (error) {
...
export const fetchMyNftList = createAsyncThunk(
  "mypage/fetchMyNftList",
  async (mypageData) => {
    console.log("axios값", mypageData);
    try {
      const result = await axios({
        method: "post",
        url: "http://127.0.0.1:8080/mypage/heartlist",
        data: mypageData,
      });
      return result.data;
    } catch (error) {
...
export const myNftList = createAsyncThunk(
  "mypage/myNftList",
  async (user_wallet_address) => {
    const web3 = new Web3(window.ethereum);
    const contract = new web3.eth.Contract(NEWSIC_FUND_ABI, NEWSIC_FUND_CA);
    try {
      const myNftListArr = await contract.methods
        .totalUri()
...
export const myRunningFundList = createAsyncThunk(
  "mypage/running-fund",
  async (user_wallet_address) => {
    try {
      console.log("통신 시작");
      const myFundingList = await axios({
        method: "post",
        url: "http://127.0.0.1:8080/mypage/creator/running-fund",
...
export const getNotice = createAsyncThunk("fund/notice/get", async (data) => {
  try {
    const getNoticeList = await axios({
      method: "post",
      url: "http://127.0.0.1:8080/fund/notice/get",
      data: data,
    });
