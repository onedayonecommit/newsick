...
//       price: await web3.utils.toWei(data.funding_price.toString(), "ether"), // 개당 가격
...
const FundingCreateContainer = () => {
  ...
  const makeFund = async () => {
    let _fundingStruct = [
      ...
      0, // 개당 가격
      data.funding_min * 2, // 최대
      data.funding_holdershare, // 음원수익(홀더)
    ];
    const _sendData_toContract = await NEWSIC_FUND.methods._setUri(_fundingStruct, data, await web3.utils.toWei(String(funding_price), "ether")).send({ from: userInfo.address });
